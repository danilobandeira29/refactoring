...
export default function createStatement(invoice: Invoice, plays: Play) {
    ...
    statement.performances = invoice.performances.map((p) => enrichPerformance(p, plays));
    statement.totalAmount = totalAmount(statement);
    statement.totalVolumeCredits = totalVolumeCredits(statement);
    return statement;
}

function enrichPerformance(aPerformance: Performance, plays: Play) {
    const calculator = createPerformanceCalculator(aPerformance, playsFor(aPerformance, plays));
...
function playsFor(p: Performance, plays: Play) {
